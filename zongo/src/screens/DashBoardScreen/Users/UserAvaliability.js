...
const UserAvaliability = ({ navigation, route }) => {
    ...
    const apiGetUserAvailabilityDetails = useSelector(state => state.userModuleRedux.apiGetUserAvailabilityDetails);
    const user_availability_details = useSelector(state => state.userModuleRedux.user_availability_details);

    const apiGetUserAvailabilityTimeDetails = useSelector(state => state.userModuleRedux.apiGetUserAvailabilityTimeDetails);
    const user_availability_time_details = useSelector(state => state.userModuleRedux.user_availability_time_details);

    const apiExtensionUpdateTimeSlot = useSelector(state => state.userModuleRedux.apiExtensionUpdateTimeSlot);
    const apiUserCreateTimeSlot = useSelector(state => state.userModuleRedux.apiUserCreateTimeSlot);
    const apiCopyUserTimeSlot = useSelector(state => state.userModuleRedux.apiCopyUserTimeSlot);
    const apiDeleteUserTimeSlot = useSelector(state => state.userModuleRedux.apiDeleteUserTimeSlot);
    ...
    const GetTimeBasedRoutingDetails = () => {
        if (user_data !== null) {
            ...
            dict.createdby = user_data?.data?.user_uuid,
                dict.user_uuid = route?.params?.user_uuid,
                dispatch(Get_User_Availablity_Details(dict))
        }
    }
    ...
    useEffect(() => {
        Log('apiGetUserAvailabilityDetails :', apiGetUserAvailabilityDetails);
        if (apiGetUserAvailabilityDetails == STATUS_FULFILLED) {
            Log("user_availability_details :", user_availability_details)
            if (user_availability_details !== null) {
                // setTimeBasedRoutingDetails(time_based_routing_details?.time_condition)
                setTimeConditionDayData(user_availability_details?.availability_data)
                // setCallScheduleName(time_based_routing_details?.time_condition[0]?.time_condition_name)
            }
        } else if (apiGetUserAvailabilityDetails == STATUS_REJECTED) {
            if (isError) {
                Alert.alert('Alert', error_message);
            }
        }
    }, [apiGetUserAvailabilityDetails]);




    useEffect(() => {
        Log('apiGetUserAvailabilityTimeDetails :', apiGetUserAvailabilityTimeDetails);
        if (apiGetUserAvailabilityTimeDetails == STATUS_FULFILLED) {
            Log("user_availability_time_details :", user_availability_time_details)
            if (user_availability_time_details !== null) {
                switch (SelectedDay?.user_availability_day) {
                    case 'monday':
                        const Monday = user_availability_time_details.find((data) => data?.user_availability_data_uuid == SelectedDay.user_availability_data_uuid)
                        if (Monday) {
                            setMondayData(user_availability_time_details)
                        }
                        break;
                    case 'tuesday':
                        const Tuesday = user_availability_time_details.find((data) => data?.user_availability_data_uuid == SelectedDay.user_availability_data_uuid)
                        if (Tuesday) {
                            setTuesdayData(user_availability_time_details)
                        }
                        break;
                    case 'wednesday':
                        const Wenesday = user_availability_time_details.find((data) => data?.user_availability_data_uuid == SelectedDay.user_availability_data_uuid)
                        if (Wenesday) {
                            setWenesdayData(user_availability_time_details)
                        }
                        break;
                    case 'thursday':
                        const Thursday = user_availability_time_details.find((data) => data?.user_availability_data_uuid == SelectedDay.user_availability_data_uuid)
                        if (Thursday) {
                            setThursdayData(user_availability_time_details)
                        }
                        break;
                    case 'friday':
                        const Friday = user_availability_time_details.find((data) => data?.user_availability_data_uuid == SelectedDay.user_availability_data_uuid)
                        if (Friday) {
                            setFridayData(user_availability_time_details)
                        }
                        break;
                    case 'saturday':
                        const Saturday = user_availability_time_details.find((data) => data?.user_availability_data_uuid == SelectedDay.user_availability_data_uuid)
                        if (Saturday) {
                            setSaturdayData(user_availability_time_details)
                        }
                        break;
                    case 'sunday':
                        const Sunday = user_availability_time_details.find((data) => data?.user_availability_data_uuid == SelectedDay.user_availability_data_uuid)
                        if (Sunday) {
                            setSundayData(user_availability_time_details)
                        }
                        break;
                    default:
        ...
        } else if (apiGetUserAvailabilityTimeDetails == STATUS_REJECTED) {
            if (isError) {
                Alert.alert('Alert', error_message);
            }
        }
    }, [apiGetUserAvailabilityTimeDetails]);
    ...
    //Create Time Slot
    useEffect(() => {
        Log('apiUserCreateTimeSlot :', apiUserCreateTimeSlot);
        if (apiUserCreateTimeSlot == STATUS_FULFILLED) {
            ...
            setDestinationTo("")
            setEventName("")
        } else if (apiUserCreateTimeSlot == STATUS_REJECTED) {
            if (isError) {
                Alert.alert('Alert', error_message);
            }
        }
    }, [apiUserCreateTimeSlot]);

    //Delete Time Slot

    useEffect(() => {
        Log('apiDeleteUserTimeSlot :', apiDeleteUserTimeSlot);
        if (apiDeleteUserTimeSlot == STATUS_FULFILLED) {
            GetBusinessHoursTimeDetails(SelectedDay)

        } else if (apiDeleteUserTimeSlot == STATUS_REJECTED) {
            if (isError) {
                Alert.alert('Alert', error_message);
            }
        }
    }, [apiDeleteUserTimeSlot])



    //copy time slot
    useEffect(() => {
        Log('apiCopyUserTimeSlot :', apiCopyUserTimeSlot);
        if (apiCopyUserTimeSlot == STATUS_FULFILLED) {
            ...
            callAllApi()
        } else if (apiCopyUserTimeSlot == STATUS_REJECTED) {
            if (isError) {
                Alert.alert('Alert', error_message);
            }
        }
            ...
            if (IsFromEdit == true) {
                ...
                if (SelectedSlot?.type == "default") {
                    ...
                    dict.createdby = user_data?.data?.user_uuid,
                        ...
                        dict.route_to = DestinationValue,
                        dict.route_to_name = DestinationTo, //1009
                        dict.route_type = RouteValue,
                        dict.to_time = to,
    ...
    useEffect(() => {
        Log('apiExtensionUpdateTimeSlot :', apiExtensionUpdateTimeSlot);
        if (apiExtensionUpdateTimeSlot == STATUS_FULFILLED) {
            GetBusinessHoursTimeDetails(SelectedDay)
            setModalVisible(!modalVisible)
            setEditOffHours(false)
        } else if (apiExtensionUpdateTimeSlot == STATUS_REJECTED) {
            if (isError) {
                Alert.alert('Alert', error_message);
            }
        }
    }, [apiExtensionUpdateTimeSlot]);
            ...
            [
                ...
                },
                {
                    ...
                    onPress: () => {
                        var dict = {
                            createdby: user_data?.data?.user_uuid,
                            user_availability_times_uuid: item?.user_availability_times_uuid
                        }
                        dispatch(Delete_User_Time_Slot(dict))
                    }
                },
            ],
            { cancelable: true },
...
export default UserAvaliability;
