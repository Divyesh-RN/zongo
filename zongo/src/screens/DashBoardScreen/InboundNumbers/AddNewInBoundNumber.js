...
import { Get_Area_Code_By_State, Get_Number, Get_Prefix, Get_States, Onboarding_Buy_Did } from '../../../redux/api/Api';
...
const AddNewInboundNumber = ({ navigation }) => {
    ...
    const apiBuyDid = useSelector(state => state.inboundRedux.apiBuyDid);
    ...
    const onPlaceOrder = () => {
        if (selectedNumbers?.length > 0) {
            ...
            var dict = {
                createdby: user_data?.data?.user_uuid,
                grand_total: user_data?.data?.plan_type == "quarterly" ? selectedNumbers?.length * Number(MONTHLY_DIGIT) * QUARTELY : selectedNumbers?.length * Number(MONTHLY_DIGIT) * YEARLY + selectedNumbers?.length * Number(SETUP_DIGIT),
                main_admin_uuid: user_data?.data?.main_uuid,
                type: "normal",
                dids: selectedNumbers,
                user_uuid: user_data?.data?.user_uuid
            }
            console.log("dict", dict)
            dispatch(Onboarding_Buy_Did(dict))
        }
        else {
    ...
    useEffect(() => {
        Log('apiBuyDid :', apiBuyDid);
        if (apiBuyDid == STATUS_FULFILLED) {
            console.log("ok")
        } else if (apiBuyDid == STATUS_REJECTED) {
            if (isError) {
                Alert.alert('Alert', error_message);
            }
        }
