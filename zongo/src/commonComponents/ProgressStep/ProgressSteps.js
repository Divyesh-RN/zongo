...
class ProgressSteps extends Component {
  ...
  renderStepIcons = () => {
    ...
    // Using Array.from to iterate over the step count
    Array.from({ length: this.state.stepCount }, (_, i) => {
      const isCompletedStep = this.props.isComplete ? true : i < this.state.activeStep;
  ...
  // Callback function from ProgressStep that passes the current step.
  setActiveStep = (step) => {
    // Guard against setting the current step higher than the total step count.
    if (step >= this.state.stepCount - 1) {
      this.setState({ activeStep: this.state.stepCount - 1 });
    }
